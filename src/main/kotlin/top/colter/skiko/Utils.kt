...
fun List<Layout>.sumWidth(): Dp = sumOf { width + modifier.margin.horizontal }
fun List<Layout>.sumHeight(): Dp = sumOf { height + modifier.margin.vertical }
fun List<Layout>.sumOf(block: Layout.() -> Dp): Dp {
    var x = 0.dp
    forEach { x += it.block() }
    return x
}

fun List<Layout>.maxWidth(): Dp = maxOf { width + modifier.margin.horizontal }
fun List<Layout>.maxHeight(): Dp = maxOf { height + modifier.margin.vertical }
fun List<Layout>.maxOf(block: Layout.() -> Dp): Dp {
    var x = 0.dp
    forEach { x = Dp.max(x, it.block()) }
    return x
}

fun <T> List<T>.ifNotEmpty(block: List<T>.() -> Unit) = if (isNotEmpty()) block() else { }
fun <T> List<T>.ifNotEmptyForEach(block: (T) -> Unit) = ifNotEmpty {
    forEach { block(it) }
}

fun <E> List<E>.sumOf(function: (E) -> Float): Float {
    var c = 0f
 ...
 * 绘制矩形阴影
 */
fun Canvas.drawRectShadowAntiAlias(r: Rect, shadow: Shadow): Canvas =
 ...
 */
fun Int.withAlpha(alpha: Float = 1f): Int {
 ...
 * 角度转弧度
 */
fun deg2Arc(deg: Int) = (deg * PI / 180).toFloat()

fun Rect.centerX() = left + width / 2
fun Rect.centerY() = top + height / 2
